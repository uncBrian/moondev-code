'''
PNL PER MINUTE BOT - looks at the pnl per minute to decide when to close

this bot looks at the 15m 20 bar sma and if price > that sma 
then it will look for long opportunities, and will buy near the vwap
if price < sma then it will look for short opportunities and will sell near the vwap
...
def trade_logic():
    ...
    while True:
        ...
        buy_threshold = vwap * 1.0005  # 0.1% above VWAP
        sell_threshold = vwap * 0.9995  # 0.1% below VWAP
        ...
        if current_price > vwap and current_price > sma and current_price <= buy_threshold:
            ...
            if bid_price is not None and position is None:
                ...
                print("Buy order placed at price:", bid_price)
            ...
            if ask_price is not None and position is None:
                ...
                print("Sell order placed at price:", ask_price)
        ...
        if position['info']['side'] != 'None':
            ...
            # print above info
            print(f"Position duration mins: {position_duration_minutes}")
            print(f"PnL: {pnl:.3f}%")
            print(f"PnL % per minute: {pnl_per_minute:.3f}%")
        ...
        print('done with loop... sleeping 60 seconds')
